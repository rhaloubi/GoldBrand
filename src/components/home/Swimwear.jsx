...
const Tshirts = () => {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        const tShirtProducts = response.data.filter(product => product.category === 'Swimwear');
  ...
  return (
    <>
      <div className="row row-title md:pl-3 z-20  text-white  " style={{ justifyContent: 'left' }}>
      <div className="col pt-4 px-2 md:py-0 md:pt-6 md:pr-3 md:px-0">
          <h2 className="m">
            <AnimatedText text="SWIMWEAR:" />
          </h2>
        </div>
      </div>
        ...
        <div ref={containerRef} className="flex cursor-grab">
          ...
          <div className="border border-white p-4 md:p-6 min-w-[250px] min-h-[350px] md:min-w-[350px] md:min-h-[456px] flex-shrink-0 flex items-center justify-center ">
            ...
            <Link
                    ...
                    onClick={() => {
                      localStorage.setItem('selectedCategory', 'Swimwear');
                    }}
                  > 
                <div className="link-content">
                  <div className="link-text" style={{ fontSize: '30px' }}>
                    <span className='hover:underline'>View All</span>
                    <span className="duplicate hover:underline">View All</span>
                  </div>
                </div>
              </Link>
            </li>
