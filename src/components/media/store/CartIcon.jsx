...
const YourComponent = () => {
    ...
    useEffect(() => {
      const tl = gsap.timeline({ repeat: -1 }); // Create a timeline with infinite looping
  
      tl.to(redCircleRef.current, {
        x: 20, // Move 30px to the right
        duration: 0.5, // Time for the right movement
        ease: 'power1.inOut', // Smooth motion
      })
      tl.to(redCircleRef.current, {
        x: 0, // Move back to the original position
        duration: 0.8, // Time for the return movement
        ease: 'bounce.out', // Add bounce effect when returning
      })
      .to(redCircleRef.current, {
        duration: 3, // Pause for 2 seconds before the loop
      });
